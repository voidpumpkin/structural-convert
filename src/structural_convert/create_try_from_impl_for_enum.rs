...
pub(crate) fn create_try_from_impl_for_enum(
    ...
    let match_branches = enum_data.variants.iter().filter_map(|variant| {
        ...
        let TryFromFieldAttributes { try_from: from_attrs, try_into: into_attrs } = TryFromFieldAttributes::from_attributes(&variant.attrs).expect("Invalid field attributes");
